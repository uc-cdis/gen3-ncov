...
def analyze_divergence(sequences, metadata, reference, mask_5p=0, mask_3p=0):
    ...
    with open_file(sequences) as fasta:
        for h, s in SimpleFastaParser(fasta):
            ...
            clusters = np.array(
                np.convolve(snps * known_true_cluster_array, np.ones(ws), mode="same")
                >= cluster_cut_off,
                dtype=int,
            )
            ...
            cluster_start.extend(
                [max(0, x - ws // 2) for x in np.where(np.diff(clusters) == 1)[0]]
            )
            cluster_end = [
            ...
            diagnostics[h] = {
                "snps": list(np.where(snps)[0]),
                "gaps": list(zip(gap_start, gap_end)),
                "gap_sum": np.sum(gaps),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--reference", type=str, required=True, help="reference sequence"
    )
    ...
    parser.add_argument(
        "--mask-from-end", type=int, default=0, help="number of bases to mask from end"
    )
    parser.add_argument(
        "--output-diagnostics",
        type=str,
    ...
    diagnostics = analyze_divergence(
        args.alignment,
        metadata,
        ref,
    ...
    with open_file(args.output_diagnostics, "w") as diag:
        ...
        for s, d in sorted(
            diagnostics.items(), key=lambda x: len(x[1]["snps"]), reverse=True
        ):
            expected_div = (
                expected_divergence(metadata[s]["date"]) if s in metadata else np.nan
            )
            diag.write(
                "\t".join(
                    map(
                        ...
                        [
                            s,
                            len(d["snps"]),
                            round(len(d["snps"]) - expected_div, 2),
                            d["no_data"],
                            d["gap_sum"],
            ...
            reasons = []
            if (
                not np.isnan(expected_div)
                and np.abs(len(d["snps"]) - expected_div) > snp_cutoff
            ):
    ...
    with open_file(args.output_flagged, "w") as flag:
        ...
        for s, msg, reasons, meta in sorted(
            flagged_sequences,
            key=lambda x: x[3].get("date_submitted", "XX"),
            reverse=True,
        ):
            flag.write(
                f"{s}\t{metadata[s]['date'] if s in metadata else 'XXXX-XX-XX'}\t{metadata[s].get('date_submitted', 'XXXX-XX-XX') if s in metadata else 'XXXX-XX-XX'}\t{msg}\n"
            )
