...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mutations", type=str, required=False, help="mutations node data file"
    )
    parser.add_argument(
        "--emerging-clades",
        type=str,
        required=True,
        help="emerging clades node data file",
    )
    parser.add_argument(
    ...
    def attach_labels(n):  # closure
        if n["name"] in spike_mutations or n["name"] in clade_labels:
            ...
            if n["name"] in spike_mutations:
                n["branch_attrs"]["labels"]["spike_mutations"] = spike_mutations[
                    n["name"]
                ]
            if n["name"] in clade_labels:
                n["branch_attrs"]["labels"]["emerging_lineage"] = clade_labels[
                    n["name"]
                ]
