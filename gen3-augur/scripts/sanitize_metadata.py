...
def resolve_duplicates(metadata, strain_field, error_on_duplicates=False):
    ...
    duplicates = metadata.loc[metadata.duplicated(strain_field), strain_field].values
    ...
    accession_fields = [
        field for field in ACCESSION_FIELDS if field in metadata.columns
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="split the given GISAID location field on '/' and create new columns for region, country, etc. based on available data. Replaces missing geographic data with '?' values.",
    )
    parser.add_argument(
        "--rename-fields",
        nargs="+",
        help="rename specific fields from the string on the left of the equal sign to the string on the right (e.g., 'Virus name=strain')",
    )
    parser.add_argument(
        "--strip-prefixes",
        nargs="+",
        help="prefixes to strip from strain names in the metadata",
    )
    parser.add_argument(
        "--error-on-duplicate-strains",
        action="store_true",
        help="exit with an error if any duplicate strains are found. By default, duplicates are resolved by preferring most recent accession id or the last record.",
    )
    ...
    if args.parse_location_field and args.parse_location_field in metadata.columns:
        ...
        metadata = pd.concat([metadata, locations], axis=1).drop(
            columns=[args.parse_location_field]
        )
    ...
    if args.strip_prefixes:
        ...
        metadata[strain_field] = metadata[strain_field].apply(
            lambda strain: re.sub(pattern, "", strain)
        )
    ...
    try:
        metadata = resolve_duplicates(
            metadata, strain_field, error_on_duplicates=args.error_on_duplicate_strains
        )
    except ValueError as e:
        print(
            f"ERROR: The following strains have duplicate records: {e}", file=sys.stderr
        )
        sys.exit(1)
