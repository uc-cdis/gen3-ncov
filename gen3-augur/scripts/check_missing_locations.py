...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="input region adjusted metadata",
    )
    parser.add_argument(
        "--colors",
        type=str,
        nargs="+",
        required=True,
        help="input region specific color file",
    )
    parser.add_argument(
        "--latlong", type=str, required=True, help="input lat-long file"
    )
    ...
    for geo_value in ["location", "division", "country"]:
        ...
        missing_color_locs = [
            loc
            for loc in locs_in_meta
            if loc not in locs_w_color and loc not in things_to_exclude
        ]
        ...
        if geo_value != "country":
            missing_latlong_locs = [
                loc
                for loc in locs_in_meta
                if loc not in locs_w_latlong and loc not in things_to_exclude
            ]
