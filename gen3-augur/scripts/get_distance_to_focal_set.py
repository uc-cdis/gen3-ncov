...
def calculate_snp_matrix(
    ...
    for record in fastafile:
        ...
        if len(s) != align_length:
            raise ValueError(
                "Fasta file appears to have sequences of different lengths!"
            )
...
def calculate_distance_matrix(sparse_matrix_A, sparse_matrix_B, consensus):
    ...
    n_comp = (
        1 * (sparse_matrix_A == 110) * ((sparse_matrix_B == 110).transpose())
    ).todense()
    d = d + n_comp
    ...
    total_differences_shared = (1 * (sparse_matrix_A > 0)) * (
        sparse_matrix_B.transpose() > 0
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alignment", type=str, required=True, help="FASTA file of alignment"
    )
    parser.add_argument(
        "--reference", type=str, required=True, help="reference sequence (FASTA)"
    )
    parser.add_argument(
        "--ignore-seqs",
        type=str,
        nargs="+",
        help="sequences to ignore in distance calculation",
    )
    parser.add_argument(
        "--focal-alignment", type=str, required=True, help="focal sample of sequences"
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
        default=10000,
        help="number of samples in the global alignment to process at once. Reduce this number to reduce memory usage at the cost of increased run-time.",
    )
    parser.add_argument(
        "--output", type=str, required=True, help="FASTA file of output alignment"
    )
    ...
    focal_seqs_dict = calculate_snp_matrix(
        focal_seqs, consensus=ref, ignore_seqs=args.ignore_seqs
    )
    ...
    chunk_count = 0
    while True:
        context_seqs_dict = calculate_snp_matrix(
            seqs, consensus=ref, chunk_size=chunk_size
        )
        ...
        mask_count_context = {
            s: len(x)
            for s, x in zip(
                context_seqs_dict["names"], context_seqs_dict["filled_positions"]
            )
        }
        ...
        d = np.array(
            calculate_distance_matrix(
                context_seqs_dict["snps"],
                focal_seqs_dict["snps"],
                consensus=context_seqs_dict["consensus"],
            )
        )
        ...
        for context_index, focal_index in enumerate(closest_match):
            minimal_distance_to_focal_set[context_seqs_dict["names"][context_index]] = (
                d[context_index, focal_index],
                focal_seqs_dict["names"][focal_index],
            )

        for seqid in minimal_distance_to_focal_set:
            fh_out.write(
                f"{seqid}\t{minimal_distance_to_focal_set[seqid][1]}\t{minimal_distance_to_focal_set[seqid][0]}\n"
            )
