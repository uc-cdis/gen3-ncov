...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="amino acid alignment",
    )
    parser.add_argument(
        "--genes", type=str, nargs="+", required=True, help="amino acid alignment"
    )
    parser.add_argument(
    ...
    translations = (
        args.translations if type(args.translations) == list else [args.translations]
    )
    ...
    for gene, translation in zip(genes, translations):
        ...
        with open(translation.replace(".fasta", "_withInternalNodes.fasta"), "w") as fh:
            for n in tt.tree.find_clades():
                ...
                node_data[n.name]["aa_muts"][gene] = [
                    f"{a}{p+1}{d}" for a, p, d in n.mutations
                ]
                fh.write(
