...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--composite", type=str, required=False, help="composite sampling"
    )
    ...
    if "region_exposure" in metadata.columns:
        metadata.loc[
            metadata.region != focal_region, "division_exposure"
        ] = metadata.region_exposure
        metadata.loc[
            metadata.region != focal_region, "country_exposure"
        ] = metadata.region_exposure
        metadata.loc[
            (metadata.region == focal_region)
            & (metadata.region_exposure != focal_region),
            "division_exposure",
        ] = metadata.region_exposure
        metadata.loc[
            (metadata.region == focal_region)
    ...
    if "division_exposure" in metadata.columns:
        metadata.loc[
            (metadata.region == focal_region) & (metadata.division_exposure.isna()),
            "division_exposure",
        ] = metadata.division

    if "country_exposure" in metadata.columns:
        metadata.loc[
            (metadata.region == focal_region) & (metadata.country_exposure.isna()),
            "country_exposure",
        ] = metadata.country
