...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pangolineages", type=str, required=True, help="pangolineages.csv"
    )
    parser.add_argument("--node_data_outfile", type=str, help="pangolineages.json")
    parser.add_argument(
        "--attribute_name",
        default="pango_lineage_local",
        help="attribute name for pangolin lineage annotations in the output JSON",
    )
    ...
    node_data = {
        "nodes": {
            row["taxon"]: {args.attribute_name: row["lineage"]}
