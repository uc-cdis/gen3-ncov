...
if __name__ == "__main__":
    ...
    group.add_argument(
        "--sequences", help="*unaligned* FASTA file of SARS-CoV-2sequences"
    )
    group.add_argument(
        "--alignment",
        help="*aligned* FASTA file of SARS-CoV-2 sequences relative to Wuhan-HU-1 with insertions removed",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="clade_assignment.tsv",
        help="tsv file to write clade definitions to",
    )
    parser.add_argument(
        "--keep-temporary-files", action="store_true", help="don't clean up"
    )
    parser.add_argument(
        "--chunk-size", default=10, type=int, help="process this many sequences at once"
    )
    parser.add_argument(
        "--nthreads", default=1, type=int, help="Number of threads to use in alignment"
    )
    ...
    while not done:
        ...
        if args.sequences:
            ...
            print(
                f"writing {len(chunk)} and the reference sequence to file '{in_fname}' for alignment."
            )
        ...
        for seq in alignment:
            ...
            if len(seq.seq) != len(ref.seq):
                ...
                ipdb.set_trace()
                print(
                    f"ERROR: this file doesn't seem aligned to the reference. {seq.id} as length {len(seq.seq)} while the reference has length {len(ref.seq)}."
                )
                sys.exit(1)
            ...
            for fname, feat in features.items():
                if feat.type != "source":
                    seq_container.sequences[fname] = {
                        i: c
                        for i, c in enumerate(safe_translate(feat.extract(seq_str)))
                    }
