...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--attribute-name",
        default="delta_frequency",
        help="name of the annotation to store in the node data JSON output",
    )
    parser.add_argument(
        "--output",
        required=True,
        help="JSON of delta frequency annotations for nodes in the given tree",
    )
    ...
    for node in tree.find_clades(order="postorder"):
        if node.is_terminal():
            ...
            node.frequencies = np.array(
                [child.frequencies for child in node.clades]
            ).sum(axis=0)
        ...
        if node.frequencies[last_pivot_index] > args.min_frequency:
            if node.count_terminals() >= args.min_tips:
                ...
                if args.method == "linear":
                    node_delta_frequency = (
                        node.frequencies[last_pivot_index]
                    ...
                    y_frequencies = logit_transform(
                        node.frequencies[first_pivot_index:], pc=0.001
                    )
                    ...
                    print(
                        f"Error: The request method, '{args.method}', is not supported.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
                ...
                delta_frequency[node.name][args.attribute_name] = delta_frequency[
                    node.parent.name
                ][args.attribute_name]
            else:
