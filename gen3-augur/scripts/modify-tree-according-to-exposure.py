...
def switch_attribute(node):
    ...
    if exposure:
        ...
        if (
            "exposure_backup" in node["node_attrs"]
            and node["node_attrs"]["exposure_backup"]["value"]
            != node["node_attrs"][EXPOSURE_TRAIT]["value"]
        ):
            node["node_attrs"][EXPOSURE_TRAIT]["value"] = node["node_attrs"][
                "exposure_backup"
            ]["value"]
...
def reset_colors(colorings, values_wanted, colors):
    ...
    for missing_name in values_wanted_lower - values_added:
        print(
            "WARNING: Colour for {} -> {} is missing & auspice will choose a shade of grey for this.".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--colors", type=str, metavar="TSV", required=True, help="input colors TSV"
    )
    parser.add_argument(
        "--lat-longs",
        type=str,
        metavar="TSV",
        required=True,
        help="input lat/longs TSV",
    )
    parser.add_argument(
        "--sampling",
        type=str,
        required=True,
        help="name of sampling location attribute",
    )
    parser.add_argument(
        "--exposure",
        type=str,
        required=True,
        help="name of exposure location attribute",
    )
    parser.add_argument(
    ...
    # on the nodes.
    input_json["meta"]["filters"] = [
        f for f in input_json["meta"]["filters"] if f != EXPOSURE_TRAIT
    ]
