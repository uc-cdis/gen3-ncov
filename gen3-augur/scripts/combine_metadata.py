...
def parse_args():
    ...
    parser.add_argument(
        "--metadata", required=True, nargs="+", metavar="TSV", help="Metadata files"
    )
    parser.add_argument(
        "--origins",
        required=True,
    ...
    parser.add_argument(
        "--output", required=True, metavar="TSV", help="Output (merged) metadata"
    )
...
if __name__ == "__main__":
    ...
    for m in metadata:
        print(
            f"\t{m['origin']} ({m['fname']}): {len(m['data'].keys())} strains x {len(m['columns'])} columns"
        )
    ...
    for metadata_entry in metadata:
        ...
        for strain in combined_data:
            combined_data[strain][origin] = (
                "yes" if strain in metadata_entry["strains"] else "no"
            )

    print(
        f"Combined metadata: {len(combined_data.keys())} strains x {len(combined_columns)} columns"
    )
