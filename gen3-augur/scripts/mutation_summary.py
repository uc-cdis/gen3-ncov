...
def read_reference(fname, genemap):
    ...
    with open(genemap, "r") as fh:
        for line in fh:
            ...
            strand = entries[6]
            attributes = {
                x.split()[0]: " ".join(x.split()[1:]) for x in entries[8].split(";")
            }
            ...
            translation = Seq.translate(
                SeqFeature.SeqFeature(
                    SeqFeature.FeatureLocation(
                        start - 1, end, strand=-1 if strand == "-" else 1
                    )
                ).extract(ref)
            )
...
def get_differences(s1, s2, ambiguous="N"):
    ...
    s2_trimmed = (
        ambiguous * (len(s2_rstrip) - len(s2_lrstrip))
        + s2_lrstrip
        + ambiguous * (len(s2) - len(s2_rstrip))
    )
    return [
        (a, p + 1, d)
        for p, (a, d) in enumerate(zip(s1, s2_trimmed))
        if d not in [a, ambiguous]
    ]
    ...
    with open_file(aln_file, "r") as fh:
        for si, (name, seq) in enumerate(SimpleFastaParser(fh)):
            if si % 1000 == 0 and si:
                print(f"sequence {si}")
            res[name] = ",".join(
                [f"{a}{p}{d}" for a, p, d in get_differences(ref, seq, ambiguous)]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--directory", type=str, required=True, help="directory with nextalign output"
    )
    parser.add_argument(
        "--alignment",
        type=str,
        required=False,
        help="nucleotide alignment (if not part of default pattern)",
    )
    parser.add_argument(
        "--insertions",
        type=str,
        required=False,
        help="insertions (if not part of default pattern)",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="list of gene names to summarize mutations for",
    )
    ...
    nucleotide_alignment = args.alignment or os.path.join(
        args.directory, args.basename + ".aligned.fasta*"
    )
    insertions = os.path.join(args.directory, args.basename + ".insertions.csv")

    genes = set(args.genes)
    gene_files = glob.glob(
        os.path.join(args.directory, args.basename + ".gene.*.fasta*")
    )

    compressed = {}
    res = to_mutations(nucleotide_alignment, ref)
    compressed = {
        "nucleotide_mutations": pd.DataFrame(
            res.values(), index=res.keys(), columns=["nucleotide"]
        )
    }
